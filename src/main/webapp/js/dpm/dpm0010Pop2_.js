...
var modDpm0010Pop = (function(){
    ...
    function objectInit() {    	
        ...
        var viewerHTML1 = 
        f_createObjectToHTML("MImgView1", "100%", "580px", MALLVIEW_OCX_CLSID, "", "");
        ...
        /// toolbar Create
        /*
        var divViewer2 = document.getElementById("divAllView2");        
        var viewerHTML2 = 
        f_createObjectToHTML("MImgView2", "100%", "55px", MALLVIEW_OCX_CLSID, "", "");
        divViewer2.innerHTML = viewerHTML2;        
        ...
        */
        ...
        var viewerHTML3 = 
        f_createObjectToHTML("MImgView3", "100%", "580px", MALLVIEW_OCX_CLSID, "", "");
    ...
    function objectEventInit()
    {
    	if(window.attachEvent) {
    		axAllViewCtlC1.attachEvent('OnSelectedImage', function(x, y, w, h){return MImgView1_OnSelectedImage(x, y, w, h); });
    	}
    	else if(window.addEventListener) {	//ie11
    		axAllViewCtlC1.addEventListener('OnSelectedImage', function(x, y, w, h){return MImgView1_OnSelectedImage(x, y, w, h); }, false);
    	}
    };
    ...
    function MImgView1_OnSelectedImage(x, y, w, h){
    	...
    	try {
			...
			// @check - Rect, roundSize 는 c#에서 struct. 우선 json data로 넘기도록 구현하였으나 에러발생함
    		// 드래그된 사각형 영역
            ...
            axAllViewCtlC1.AddBlackMarkBoxEx(Rect.Left,Rect.Top,Rect.Right,Rect.Bottom);
            //axAllViewCtlC1.EditAnnotationBOXbyIndex(axAllViewCtlC1.getAnnotationCount - 1, Rect, 16777215, 16777215, 0, roundSize, 0, 0, 13, 1);
            //axAllViewCtlC1.SelectImageMode = 0;
            ...
            if(!confirm("해당 영역에 수동 마스킹을 진행하시겠습니까?")) {
            	...
            	if(gMaskFile == "" || gMaskFile == "gitar4") {
                	...
                	if(!objCommUtil.ExistFile(oname)) {
                		objCommUtil.FileCopy(IMG_PATH, fpath, true);
                	}
            	...
            	if(objCommUtil.ExistFile(gMaskFile)) {
            		...
            		for(i=0; i<axAllViewCtlC1.getAnnotationCount; i++)
            		{
            			axAllViewCtlC1.getAnnotationInfo(i, aType, isSelected, ObjectIndex, Rect, BufferSize);
            			arriRect = [Rect.Left, Rect.Top, Rect.Right, Rect.Top, Rect.Right, Rect.Bottom, Rect.Left, Rect.Bottom];
            			
            			// 드래그된 사각형 영역 영구주석화
            			// @check - DrawPolyline 어떻게?  (MagicMasking.dll import?
            			iRet = DrawPolyline(modComm.stringToUnicodeArray(gMaskFile), modComm.stringToUnicodeArray(gMaskFile), arriRect);
            		}
            		
            		for(j=0; j<axAllViewCtlC1.getAnnotationCount; j++)
            		{
            			axAllViewCtlC1.RemoveAnnotationbyIndex(axAllViewCtlC1.getAnnotationCount - 1);
            		}
                    ...
                    if (gMaskingCount > 0)
                    {
                    	axAllViewCtlC1.SaveImage();
                    }
                    
                    axALLViewFileManagerCtlC1.SetImageFile = axAllViewCtlC1.ImageFilePath;
                    axALLViewFileManagerCtlC1.InitSelectionAreaofImage();
                    axALLViewFileManagerCtlC1.AddSelectionAreaofImage(Math.round(x), Math.round(y), Math.round(w), Math.round(h));
                    axALLViewFileManagerCtlC1.SaveSelectionAreaofImage(sname, 1);
            	}
            	
            	gMaskFile = axAllViewCtlC3.ImageFilePath;
    ...
    function MaskingCancel() {
    	if(!confirm("기 마스킹 이미지를 원상복귀 진행하시겠습니까?")) return;
    ...
    function checkBeforeClose() {
        ...
        if(!bSendState)
        {
            ...
            for (i = 0; i < iImgCnt; i++)
            {
                sPath = axAllViewCtlC1.GetThumbnail_FilePath(i); // 이미지경로 가져오기
                iAnnoCnt = axALLViewFileManagerCtlC1.GetAnnotationCount(sPath, -1); // 주석 개수 가져오기(마스킹)
