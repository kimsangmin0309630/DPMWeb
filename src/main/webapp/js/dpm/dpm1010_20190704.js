...
//    	            return (cm[i].name === 'cb');
                    ...
                    if (targerElementId.length == 16){
                    	...
                    	if ((isOri == "O" &&  (crtDt >= CONVERTDAY)) || $("#selVerifyYn").val() == "3"){
                    		if (!objCommUtil.ExistFile(sFile)){
                    			bResult = objCommUtil.xMaskImgDown(targerElementId, sFile,"V",XMASK_TARGET);   //다운로드    
                            }
...
//                    if (format == 9){
//                    	objCommUtil.gFun_Bar("이미지 로딩 중입니다.@@잠시만 기다려 주세요.");
//            			axALLViewFileManagerCtlC1.ConvertPDF2TIFF(sFile,sFile,3.0,7,100);
//                    	axALLViewFileManagerCtlC1.SetImageFile = sFile;
//
//            		}
...
//        	//pSubLoadImgView(sOrigin, sMask, sIDXID, sEID, sJobID, sThumbnail);
		...
		//DB 정보 업데이트 
		ImageManualMaskingResultSave(sEID, "","","","",0,true,0,sJobID,sServer,sAgent);
...
function ImgVerify() {
	...
	if (!confirm("대상 건을 검증하시겠습니까?")) {
		...
		for (var i = 0; i < ids.length; i++){
        	...
        	if (sMaskPrgStsc == "미검증"){
        		$('#jqGrid').setCell(ids[i],"verifySts","완료"); 
	        	if (sMaskEID != undefined && sMaskEID != "" && sMaskEID !=null ){
	        		...
	        		ImageManualMaskingVerifySave(sEID,"","","","",0, false, sJobID,sServer,sAgent);
	        	}
...
function gFunImgMasking(gOrigin, gMaskEID, gIDXID, gEID, gJobID, gThumbnail, gThumbnailPath,sServer,sAgent){
	...
	try {
		...
		console.log("format : " + format.toString());
//		if (format == 9){
//			axALLViewFileManagerCtlC1.ConvertPDF2TIFF(IMG_PATH,IMG_PATH.replace("." +item.etc2,".tif"),3.0,7,100);
//		}
	...
	//						.SaveSelectionAreaofImage(sname2, 1);
		...
		if (fpCn != 0){
			...
			ImageManualMaskingResultSave(gEID, "","","","",0,true,maskPageCount,gJobID,sServer,sAgent);
		}
